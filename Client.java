 ...
 * - Tweeking weights of weekend call and weights of deviation from avg call factor
 * - Adding more rules
 * - make an assignCall function that will call the usual 4 functions when assigning call (add to overal call list, add to res call list, q3, weekends)
 *
 * 
 * Recently added:
 * - Hard max 2 weekend calls
 * - Cannot be scheduled for two consecutive weekends in a month
 * - Give electives two Sat calls before giving GOR a third weekend call
 * */
...
public class Client
{
 public static void main(String args[]) throws java.io.IOException 
 {
   ...
   for(int count=0;count<repeatCount;count++)  // Make x number of versions of the randomly generated call schedule.
   {
     ...
     callSchedule = preFillElective(Residents,callSchedule,myCalendar,sat,1,1,"VA");
     callSchedule = preFillElective(Residents,callSchedule,myCalendar,sat,1,1,"Kernan");
     callSchedule = preFillElective(Residents,callSchedule,myCalendar,sat,1,1,"NeuroMon");
     callSchedule = preFillElective(Residents,callSchedule,myCalendar,sat,1,1,"UMRAS");
     callSchedule = preFillElective(Residents,callSchedule,myCalendar,sat,1,1,"ChronicPain");
     
     callSchedule = preFillOB(Residents,callSchedule,myCalendar,monThurs,4);    
     callSchedule = preFillWeekend(Residents,callSchedule,myCalendar,friSatSun,Calendar.SUNDAY);
     callSchedule = preFillWeekend(Residents,callSchedule,myCalendar,friSatSun,Calendar.FRIDAY);
     callSchedule = preFillWeekend(Residents,callSchedule,myCalendar,friSatSun,Calendar.SATURDAY);
 
     callSchedule = preFillElective(Residents,callSchedule,myCalendar,sat,1,2,"VA");
     callSchedule = preFillElective(Residents,callSchedule,myCalendar,sat,1,2,"Kernan");
     callSchedule = preFillElective(Residents,callSchedule,myCalendar,sat,1,2,"NeuroMon");
     callSchedule = preFillElective(Residents,callSchedule,myCalendar,sat,1,2,"ChronicPain");
 ...
 //  displayAllCall(efficientResidents, avgCall(efficientResidents));
   ...
   displayMyCall("Formanek",efficientResidents, avgCall(efficientResidents));
   displayMyCall("Cardillo",efficientResidents, avgCall(efficientResidents));
   displayMyCall("Neary",efficientResidents, avgCall(efficientResidents));
   displayMyCall("Chow",efficientResidents, avgCall(efficientResidents));
...
public static String[][] preFillWeekend(Person[] myResidents, String[][] myCall, int[] myCalendar, int[] myDaysOfWeek, int dayToFill) throws java.io.IOException
 {
    ...
    { 
       if(myDaysOfWeek[day]!=-1 && myCalendar[myDaysOfWeek[day]]==dayToFill)
       { 
          ...
          for(int callType=0;callType<=3;callType++)  // Cycle through OB, OR1, OR2, and OR3 call on that day
          {
             ...
             if(residentChosen>=0)
             {
                ...
                noConsecutiveWeekends(myResidents[order[residentChosen]],myCalendar,myDaysOfWeek[day]);
 ...
 public static String[][] preFillElective(Person[] myResidents, String[][] myCall, int[] myCalendar, int[] myDaysOfWeek,int numCallsToAssign,int maxCalls,String myRotation) throws java.io.IOException
 {
   ...
   for(int res=0;res<myResidents.length;res++)  // Go through each resident.
   {
     if(myResidents[res].getRotation()==myRotation)  // Find a resident on the specified rotation.
     {
       ...
       myResidents[res].setMaxTotalCall(maxCalls);
       ...
       for(int count=1;count<=numCallsToAssign;count++)
       {
         outerLoop:
         ...
         if(dayChosen>=0)
         {
           ...
           noConsecutiveWeekends(myResidents[res],myCalendar,dayChosen);  // Make this resident ineligible for call the next/prev weekend
     ...
     if(myResidents[res].getRotation()=="OB")  // Find a resident on the OB rotation.
     {
       ...
       for(int count=1;count<=numCalls;count++)
       {
         ...
         if(dayChosen>=0)
         {
           ...
           maxQ3(myResidents[res],dayChosen); // Make this resident ineligible for call for the next 2 days
           noConsecutiveWeekends(myResidents[res],myCalendar,dayChosen);
    ...
    { 
      ...
      for(int callType=0;callType<=3;callType++)  // Cycle through OB, OR1, OR2, and OR3 call on that day
      {
          ...
          if(residentChosen>=0)
          {
            ...
            noConsecutiveWeekends(myResidents[order[residentChosen]],myCalendar,day);
    ...
    return (
            ...
            && 
               ((callType==0) ||                                          // OB call, do nothing
                  ...
                  ((callType==2 && myResidents[resNum].getRank()>=2)) ||  // OR 2 call must be a CA-2 or CA-3
 ...
 public static void displayOrder(int A[]) throws java.io.IOException
 {
  ...
  {  
    // Make ineligible for next 2 days
  ...
  public static void noConsecutiveWeekends(Person singleResident, int[] myCalendar, int myDay)
  {  
     int nextWeekend=-1, flag=0, weekendFound=0;
     
     if(myCalendar[myDay]==1 || myCalendar[myDay]==6 || myCalendar[myDay]==7)
     {
        // Make ineligible for following weekend.
        for (int cv=myDay+1;cv<myCalendar.length;cv++)
        {
           if(myCalendar[cv]>=2 && myCalendar[cv]<=5)  // Keep cycling thru each day until it finds the next non-weekend day (i.e. monday)
              flag=1;
           
           if( (myCalendar[cv]==1 || myCalendar[cv]==6 || myCalendar[cv]==7) && flag==1)  // Find the next weekend day after the flag is set and make the resident ineligible.
           {
              singleResident.setgorEligible(cv,false);
              singleResident.setobEligible(cv,false);
              weekendFound=1;
           }
           
           if( (myCalendar[cv]>=2 && myCalendar[cv]<=5) && weekendFound==1)
              break;
        }
        
        flag = weekendFound = 0;
        
        // Make ineligible for previous weekend.
        for (int cv=myDay-1;cv>=0;cv--)
        {
           if(myCalendar[cv]>=2 && myCalendar[cv]<=5)  // Keep cycling thru each day until it finds the next non-weekend day (i.e. Thurs)
              flag=1;
  ...
  {
    System.out.println("\n*********************************************");
    System.out.printf("%-12s %-11s %-3s %-4s %-8s %-8s %-8s %-8s %-6s %-12s %n","Name", "Rotation", "OB", "GOR", "Weekday", "Weekend","Friday", "Sat/Sun", "Total", "Call Factor");
    ...
    for(int cv=0;cv<myResidents.length;cv++)
    {
       System.out.printf("%-12s %-11s %-3s %-4s %-8s %-8s %-8s %-8s %-6s %-12s %-10s %n",
                         ...
                         myResidents[cv].getMaxSatSunCall()
                        );
    }
  }
   
   public static int findResident(Person[] myResidents, String myName)
   {
      int location=-1;
      for(int res=0;res<myResidents.length;res++)  // Go through each resident.
      {
         if(myResidents[res].getName()==myName)
         {
            location=res;
            break;
         }
   ...
   public static void displayDaysEligible(Person[] myResidents, int[] myCalendar, String myName)
   {
      int location=findResident(myResidents,myName);
      myResidents[location].displayDaysEligible(myCalendar);
   }
